...
The current Profound documentation system seems to support document outlines natively, and so does GitHub, so I chose not to include any tables of contents in my submission documents. To view the content outline for each document, you can click on the outline icon on the top right of the document view.
...
The contents of these documents are based on the screenshots I received in the test assignment spec, other Profound features, existing Profound documentation, as well as competitor analysis (seeing what other companies do with similar features).
...
Also, some documents are not as detailed as I prefer my documents to be. For example:

- The Integration Tutorial is missing details about selecting search results when integrating information from Knowledge Bases to Workflows, because I wasn't sure how that would work and did not want to invent the entire UI experience.
- The Integration Tutorial's "Create Workflow" step includes the suggested steps but does not give a concrete example of a functioning workflow with extensive details like the exact prompts, etc. The reason for this is the limited time I could spend familiarizing myself with Workflows: I did try to put a Workflow together with some mock steps, but I wasn't sure if my example would be good enough to be useful for a hypothetical reader, so I opted out for a more general guideline.

In real-life work conditions, I would certainly avoid assumptions and missing details by testing the in-development feature using a test account, clarifying what I don't know with Engineering, Marketing, Sales, or other teams, or sitting down with Product Owners to figure out the best use cases for features before including them in the customer-facing tutorials.
